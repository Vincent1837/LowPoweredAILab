...
if __name__ == "__main__":
    ...
    print(f"Input shape: {x_train_reduced.shape}")
    ...
    train_hop3_feats_plus = get_feat(x_train_reduced, p2_plus).reshape(len(x_train_reduced), -1)
    test_hop3_feats_plus = get_feat(x_test, p2_plus).reshape(len(x_test), -1)
    
    
    # --------- DFT
    from feat_utils import feature_selection
    print(f"Selecting Features from training set...")
    print(f"Training set shape: {train_hop3_feats_plus.shape}")
    selected, dft_loss = feature_selection(train_hop3_feats_plus, y_train_reduced, FStype='DFT_entropy', thrs=0.7, B=16)
    train_plus_reshaped = train_hop3_feats_plus.reshape(len(train_hop3_feats_plus), -1) [:, selected]

    test_plus_reshaped = test_hop3_feats_plus.reshape(len(test_hop3_feats_plus), -1) [:, selected]
    with open("./dft_selected.pickle", "wb")as f:
        pickle.dump(selected, f)
        
    with open("./dft_loss.pickle", "wb") as f:
        pickle.dump(dft_loss, f)
