# -*- coding: utf-8 -*-
#==========================================================================#
# Description: FMCW RADAR Doppler processing                               #
...
def doppler_resolution_max_speed(configs):
    """Calculate the doppler resolution and maximum speed for the given radar configuration.
    ...
    wave_length = configs.LIGHT_SPEED / configs.center_frequency_Hz
    max_speed = wave_length / (4 * configs.chirp_repetition_time_s)
    #doppler_resolution = wave_length / (2 * configs.frame_repetition_time_s)
    doppler_resolution = max_speed / (configs.num_chirps_per_frame/2)
    
    return doppler_resolution, max_speed, wave_length
...
def doppler_processing(radar_cube,
    ...
    if interleaved:
        ...
        # Antennas associated to tx1 (Ping) are 0:4 and to tx2 (Pong) are 4:8.
    ...
    # FFT 32x32
    fft2d_out = np.fft.fft(fft2d_in, n=n_fft)
    aoa_input = fft2d_out
    ...
    # Absolute Value
    #fft2d_log_abs = np.abs(fft2d_out)

    # Power
    fft2d_log_abs = np.abs(fft2d_out)**2
    
    # Power in dB
    #fft2d_log_abs = 10*np.log10(np.abs(fft2d_out)**2)
