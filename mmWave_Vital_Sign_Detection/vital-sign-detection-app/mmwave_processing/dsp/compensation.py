...
def exponential_smoothing(input_val, pre_val, init, alpha=0.02):
    """Perform exponential smoothing from the input_val on the specified doppler axis for static clutter removal.
    ...
    # In the beggining
    if init:
        output_val[0] = input_val[0]
    else:
        output_val[0] = pre_val
    
    # Apply exponential smoothing
    for i in range(1, input_val.shape[0]):
        output_val[i] = alpha * input_val[i] + (1 - alpha) * output_val[i-1]
    
    pre_val = output_val[-1]
    
    return output_val, pre_val

def dc_compensation(radar_cube):
    """The center and the radius of the radar point cloud can be estimated based on a non-linear least square estimation (NLLS),
    ...
    dc_compensated = np.zeros_like(radar_cube)
    A = np.ones([radar_cube.shape[0], radar_cube.shape[1], radar_cube.shape[2], 3])
    b = np.zeros([radar_cube.shape[0], radar_cube.shape[1], radar_cube.shape[2]])
    y = np.zeros([radar_cube.shape[0], radar_cube.shape[1], 3])
    for i in range(radar_cube.shape[0]):
        for j in range(radar_cube.shape[1]):
            A[i, j, :, 1] = -2*radar_cube[i, j, :].real
            A[i, j, :, 2] = -2*radar_cube[i, j, :].imag
            b[i, j, :] = -(radar_cube[i, j, :].real**2 + radar_cube[i, j, :].imag**2)
    
    for i in range(radar_cube.shape[0]):
        for j in range(radar_cube.shape[1]):
            y[i,j]=np.matmul(np.matmul(np.linalg.inv(np.matmul(A[i,j,:,:].T,A[i,j,:,:])),A[i,j,:,:].T),b[i,j,:]) 
    
    for i in range(radar_cube.shape[2]):
        dc_compensated[:, :, i] = radar_cube[:, :, i] - (y[:,:,1] + 1j*y[:,:,2])
    
    return dc_compensated
